...
from silver_speak.utils import encode_text, loglikelihood, replace_one_character, decode_tokens
def decrease_loglikelihood_replace_characters_by_equivalents(chars_map, text):
    ...
    for i in range(15):
        new_tokens_list = replace_one_character(chars_map, loglikelihoods)
        loglikelihoods = loglikelihood(new_tokens_list)
        print(f'loglikelihoods: {loglikelihoods}')
    
    # Reconstruct the text
    text = decode_tokens(new_tokens_list)
    print(f'Mean ending loglikelihood: {sum([x[1] for x in loglikelihoods]) / len(loglikelihoods)}')
    return text
...
def rewrite_attack(text, do_replace_chars=True, do_replace_spaces=True):
    rewritten_text = text
    if do_replace_chars:
        rewritten_text = decrease_loglikelihood_replace_characters_by_equivalents(chars_map, rewritten_text)
