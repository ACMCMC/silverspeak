...
def targeted_attack(
    ...
    """
    Replace a percentage of characters in text with context-aware homoglyphs.
    ...
    The function uses context-aware scoring to select optimal homoglyphs that match
    the surrounding text properties, resulting in more natural-looking replacements.
    The implementation selects the highest scoring homoglyph replacements based on
    context matching and Unicode property analysis.

    Args:
        text (str): The input text to transform.
        percentage (float, optional): The percentage of characters to replace (0.0-1.0).
            Defaults to 0.1 (10%).
        context_window_size (int, optional): Size of context window to analyze around each
            character for property matching. Defaults to 10.
    ...
    Returns:
        str: The transformed text with optimized homoglyph replacements.
    ...
    Example:
        ```python
        # Replace 5% of characters with context-matched homoglyphs
        ...
        # Replace 10% of characters with a different context window size
        modified_text = targeted_attack(
            "Hello world",
            percentage=0.1,
    ...
    for i, char in enumerate(text):
        if char in chars_map:
            # Extract context around the character for property matching
            start = max(0, i - context_window_size // 2)
            end = min(len(text), i + context_window_size // 2 + 1)
            context_chars = text[start:end]
            ...
            for homoglyph in chars_map[char]:
                ...
                # Score the homoglyph based on context match
                score = replacer.score_homoglyph_for_context(
                    homoglyph=homoglyph,
                    char=char,
    ...
    result = ''.join(chars)
    logger.info(f"Applied {applied_replacements} context-aware homoglyph replacements")
