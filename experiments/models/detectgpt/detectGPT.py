...
# Extracted from https://github.com/BurhanUlTayyab/DetectGPT/blob/main/model.py
...
class GPT2PPLV2:
    def __init__(self, device="cuda", model_id="gpt2"):
        ...
        self.t5_model = (
            transformers.AutoModelForSeq2SeqLM.from_pretrained("t5-base")
            .to(device)
            .half()
        )
    ...
    def unmasker(self, text, num_of_masks):
        ...
        results = self.t5_tokenizer.batch_decode(
    ...
    def getGeneratedTexts(self, args):
        ...
        mask_texts, list_num_of_masks = self.multiMaskRandomWord(
            original_text, ratio, n
        )
    ...
    def mask(self, original_text, text, n=2, remaining=100):
        ...
        out_sentences = pool.map(
            self.getGeneratedTexts, [(original_text, n) for _ in range(remaining // n)]
        )
    ...
    def getScore(self, sentence):
        ...
        remaining = 50
        sentences = self.mask(
            original_sentence, original_sentence, n=50, remaining=remaining
        )
    ...
    def call_1_1(self, sentence, chunk_value):
        sentence = re.sub(
            "\[[0-9]+\]", "", sentence
        )  # remove all the [numbers] cause of wiki
        ...
        for line in lines:
            ...
            if score > self.threshold:
                labels.append(1)
                prob = "{:.2f}%\n(A.I.)".format(
                    normCdf(abs(self.threshold - score)) * 100
                )
                probs.append(prob)
            else:
                labels.append(0)
        ...
        logger.debug(
            f"probability for {'A.I.' if label == 0 else 'Human'}:",
            "{:.2f}%".format(mean_prob),
        )
        return {
            "score": mean_prob,
            "generated": self.getVerdict(mean_score),
    ...
    def call_1(self, sentence):
        ...
        total_valid_char = sum(
            [len(x) for x in total_valid_char]
        )  # finds len of all the valid characters a sentence
        ...
        print(
            f"Perplexity per line {sum(Perplexity_per_line)/len(Perplexity_per_line)}"
        )
        results["Perplexity per line"] = sum(Perplexity_per_line) / len(
            Perplexity_per_line
        )
